...
namespace TucaAPI.src.Services.Portfolio
{
    public class GetStocksFromUserPortfolioService
        : IService<string, SuccessApiResponse<List<Stock>>>
    {
        ...
        public async Task<SuccessApiResponse<List<Stock>>> ExecuteAsync(string data)
        {
            var hasUser = await this.userManager.FindByEmailAsync(data.GetNonNullable());
            if (hasUser is null)
